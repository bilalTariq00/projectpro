...
import { Search, Plus, Calendar, Clock, MapPin, Tag, MoreVertical, Filter, SortDesc, Check, X, Bell, CalendarPlus, Clipboard, XCircle } from "lucide-react";
...
export default function Jobs(props: any) {
  ...
  const { data: jobs = [], isLoading: isJobsLoading } = useQuery<JobData[]>({
    ...
    queryFn: async () => {
      const response = await mobileApiCall('GET', '/api/mobile/all-jobs');
  ...
  const { data: clients = [] } = useQuery({
    ...
    queryFn: async () => {
      const response = await mobileApiCall('GET', '/api/mobile/all-clients');
  ...
  const filteredJobs = enrichedJobs.filter(job => {
    ...
    if (isRegistrationPage && (job.status === "completed" || job.status === "cancelled")) {
      return false;
    }
  ...
  const handleNewJobSubmit = async (formData: FormData) => {
    try {
              const response = await mobileApiCall("POST", "/jobs", formData);
                ...
                <DropdownMenuItem
                  ...
                  onClick={() => {
                    ...
                    if (confirm(t('mobile.jobs.actions.confirmCancel'))) {
                      ...
                      mobileApiCall('PATCH', `/api/jobs/${job.id}`, { status: 'cancelled' })
                      .then(response => {
                        if (!response.ok) throw new Error(t('mobile.jobs.actions.cancelError'));
                        return response.json();
                      })
                      .then(() => {
                        ...
                        toast({
                          ...
                          description: t('mobile.jobs.actions.cancelSuccessDesc'),
                        });
  ...
  const pageTitle = isRegistrationPage || location === '/mobile/registration' ? 'Registrazioni' : 'Lavori';
    ...
    <MobileLayout 
      ...
      rightAction={
        <div className="flex items-center">
          ...
          >
            {isRegistrationPage ? <CalendarPlus size={18} /> : <Plus size={18} />}
          </Button>
        </div>
      }
    >
            ...
            <>
              <div className="mb-2">
                <p className="text-sm text-gray-500">
                  {sortedJobs.length} {sortedJobs.length === 1 ? 'lavoro trovato' : 'lavori trovati'}
                </p>
              </div>
